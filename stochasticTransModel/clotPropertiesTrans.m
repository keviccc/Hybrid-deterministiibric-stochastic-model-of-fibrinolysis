...
function [FDP_dist,extentLysis,epsilon] =  clotPropertiesTrans(X, CSActive, lysisLevel)

    global n_0 epsilon_0 count U_0 N_CS N_AV 
	
    nCS = length(CSActive);
    threshold = n_0*U_0*1E-6*1E-15*N_AV/N_CS*lysisLevel 
	...
	for it = 1 : nCS
		...
		if (X(it + 3*nCS) < threshold && CSActive(it) == 1)
