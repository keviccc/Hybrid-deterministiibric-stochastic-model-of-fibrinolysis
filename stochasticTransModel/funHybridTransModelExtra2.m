...
function [t,ploti] = funHybridTransModelExtra2(mu,sigma,rho_0,U_new,finalTime,nt,C_0,N_fibres,lysisLevel,alpha_new, phi_p0)
...
global U_0 check N_old extentLysis alpha
...
r_cell = 1; % um
V_cell = (4/3)*pi()*r_cell^3; % um^3

cell_0 = phi_p0/V_cell; % num_cell/um^3
...
ploti.cell = zeros(1,nt);
ploti.phi_p = zeros(1,nt);
ploti.phi_f = zeros(1,nt);
...
ploti.phi_p(1) = phi_p0;
ploti.phi_f(1) = phi_f0;
...
for it = 2:nt
    ...
    C_input = [Cs_tPA; Cs_PLG; Cs_PLS; cell];
    ...
    sol = rungeKuttaFourth2(C_input, current_t, delta_t);
    ...
    [FDP_dist,extentLysis,epsilon] =  clotPropertiesTrans(X, CSActive,lysisLevel);
    phi_p = cell*V_cell;
    ...
    phi_f = phi_f0*(store.ns_tot/n_0);
    
    if store.ns_tot/ploti.ntot(1) < 0.05
       phi_f = 0;
    ...
    ploti.cell(it) = cell;
    ploti.phi_p(it) = phi_p;
    ploti.phi_f(it) = phi_f;
